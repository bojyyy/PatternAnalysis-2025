...
- Engineers compact, stationary features:
    one-hot type {1,2,3,4,5,7}, side in {0,1}, mid_delta_ticks, spread_ticks, log1p(size), log1p(delta t ms)
- Scales continuous columns only, to [0,1]
- Yields windows without holding all windows in RAM
...
# LOBSTER datasets are headerless, hence these helpers
...
# Some preprocessing
TYPE_CLASSES = [1,2,3,4,5,7]      # Choosing to keep 7, even though trading halts should be rare in the dataset
CONT_KEYS = ["mid_delta_ticks", "spread_ticks", "log_size", "log_dt"]
TICK_SIZE = 1.0
CLIP_TICKS = 50
...
# Minmax scalar for continuous features
@dataclass
...
def build_loaders(messages_csv: str,
                  ...
                  step: int = 50,
                  batch_size: int = 128,
                  shuffle_train: bool = True) -> Tuple[DataLoader, DataLoader, DataLoader, ContinuousMinMax, Dict[str,int]]:
    ...
    X_train, feat_idx = preprocess(msgs_train, ob_train)
    cont_idx = [feat_idx[k] for k in CONT_KEYS]
    scaler = ContinuousMinMax(cont_idx).fit(X_train)

    # Datasets
    ds_tr = LOBWindowDataset(messages_csv, orderbook_csv, depth, seq_len, step, 0, train_end, scaler)
    ds_va = LOBWindowDataset(messages_csv, orderbook_csv, depth, seq_len, step, train_end, val_end, scaler)
    ds_te = LOBWindowDataset(messages_csv, orderbook_csv, depth, seq_len, step, val_end, None, scaler)
